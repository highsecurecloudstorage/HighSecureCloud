...
from Modules import login_check
        ...
        if password == cpassword:
            if user_form.is_valid() and profile_form.is_valid():
                ...
                if request.POST.get('trustedsystem')=='Y':
                    ...
                    MacAdd.provincial=r.split('short_name')[4].split('long_name')[1].split('"')[2]
                    MacAdd.country=r.split('short_name')[5].split('long_name')[1].split('"')[2]
                    MacAdd.PIN=int(r.split('short_name')[6].split('long_name')[1].split('"')[2])
                    MacAdd.save()
                    profile.save()
...
def dashboardShare(request):
    ...
    context = RequestContext(request)
    return render_to_response('dashboardShare.html',{}, context)
...
def dashboardViewFile(request):
    ...
    try:
        return render_to_response('viewFile.html',{'error':error,'filename':FileName,'filesize':FileSize,'fileshare':fileshare}, context)
    except:
